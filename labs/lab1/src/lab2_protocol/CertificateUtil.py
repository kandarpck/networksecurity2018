    ...
    def validate_certificate_chain(self, certs):
        ...
        for idx in range(len(cert_obj) - 1):
            if cert_obj[idx].issuer != cert_obj[idx + 1].subject:
                logger.error("Issuer and subject do not match {} {}".format(cert_obj[idx].issuer, cert_obj[idx + 1].subject))
                return False
            try:
                #if not
...
class ClientCertificateUtils(CertificateUtils):
    def __init__(self, ip_addr):
        ...
        self.intermediate_cert = self.read_certificate_from_path(os.path.dirname(__file__) + '/certificates/temp_int_cert.cert')
        self.client_cert = self.read_certificate_from_path(os.path.dirname(__file__) + '/certificates/temp_client_cert.cert')
...
class ServerCertificateUtils(CertificateUtils):
    def __init__(self, ip_addr):
        ...
        self.intermediate_cert = self.read_certificate_from_path(os.path.dirname(__file__) + '/certificates/temp_int_cert.cert')
        self.server_cert = self.read_certificate_from_path(os.path.dirname(__file__) + '/certificates/temp_server_cert.cert')
